...
export default function ConfirmMentorshipPage() {
  ...
  if (!mentor) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center space-y-4">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-[#c3c6d7] hover:text-[#b4c5ff] transition-colors text-sm group"
        >
          <ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
          <span>Voltar para Mentores</span>
        </button>
        <div className="text-center py-16">
          <p className="text-lg font-medium text-[#dae2fd]">Mentor nao encontrado</p>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4">
      ...
      {/* Main Card */}
      <div className="w-full max-w-2xl bg-[#060e20] border border-[#434655] rounded-xl p-6 md:p-8 relative overflow-hidden">
        {/* Subtle atmospheric gradient */}
        <div className="absolute -top-24 -right-24 w-64 h-64 bg-blue-600/10 blur-[60px] rounded-full pointer-events-none" />

        <header className="mb-8 relative z-10">
          <h1 className="text-[28px] md:text-[32px] leading-[36px] md:leading-[40px] font-semibold tracking-[-0.01em] text-[#dae2fd] mb-2">
            Confirmar Mentoria
          </h1>
        ...
        <section className="bg-[#131b2e] border border-[#2d3449] rounded-lg p-5 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-5 relative z-10">
          <Avatar
            src={mentor.image}
            name={mentor.name}
            size="lg"
            className="w-20 h-20 rounded-full border-2 border-[#2d3449]"
          />
          <div className="flex-1">
            <h2 className="text-lg leading-6 font-semibold text-[#dae2fd] mb-1">
              {mentor.name}
            </h2>
            {mentor.headline && (
              <p className="text-sm text-[#b4c5ff] mb-2">{mentor.headline}</p>
            )}
            {mentor.skills.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {mentor.skills.slice(0, 5).map((s) => (
                  <span
                    key={s.id}
                    className="px-2 py-1 bg-[#2d3449] text-[#c3c6d7] text-xs font-medium rounded-full"
                  >
                    {s.skill.name}
                  </span>
                ))}
        ...
        >
          <div className="flex flex-col gap-2">
            <label
              className="text-xs font-medium tracking-[0.05em] text-[#c3c6d7] flex justify-between items-end"
              htmlFor="intro-message"
            >
              <span>Mensagem de Introducao</span>
              <span className="text-[#8d90a0] text-xs opacity-70">Opcional</span>
            </label>
            <div className="relative rounded-lg transition-shadow focus-within:shadow-[0_0_0_2px_rgba(37,99,235,0.15)]">
              <textarea
                ...
                rows={4}
                className="w-full bg-[#060e20] border border-[#434655] focus:border-blue-600 text-[#dae2fd] text-sm rounded-lg p-4 resize-y focus:ring-0 transition-colors placeholder:text-[#c3c6d7]/40"
              />
            </div>
          ...
          <div className="pt-4 border-t border-[#2d3449] flex flex-col sm:flex-row justify-end gap-4">
            <button
              type="button"
              onClick={() => router.back()}
              disabled={submitting}
              className="px-6 py-3 border border-[#2d3449] text-[#dae2fd] hover:bg-[#222a3d] text-xs font-medium tracking-[0.05em] rounded-lg transition-colors active:scale-95"
            >
              Cancelar
            </button>
            <button
